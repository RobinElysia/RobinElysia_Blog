...
- **当前形态（以代码为准，2026-08-20 核对）**：`src/app/page.tsx` → `HomeScenes`：局部滚动容器（`data-scroll-container`，`snap-y snap-mandatory` 原生快吸附）+ Scene 1 全屏 3D 波浪 Hero（`wave-ocean.tsx`）+ Scene 2 逐卡翻页（`scene-carousel.tsx`，每卡一屏）+ 迷你页脚。**注意：这与设计文档早期「禁止多屏 snap/3D 轮播」的表述不同——以当前代码形态为准，改动首页结构需经用户确认。**
