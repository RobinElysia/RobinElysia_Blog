...
### Added
- **音乐播放页（用户要求）**：导航栏 Disc3 图标点击 → **圆环径向开屏**——音乐页是根布局**全屏 overlay**（非路由，旧页面保持挂载），以图标为圆心 `clip-path: circle()` 半径向外扩张（0.65s），圆内逐步露出音乐页内容、旧页面在圆外可见，直至覆盖全屏；**再点同一图标**同一圆反向缩小收回，回到上一页原样（滚动位置保留）；1px line 圆环边界随半径同步。overlay 内容图录风格——正在播放区（编号 + 衬线曲名 + 作者小字宽字距 + 墨线进度点击跳转 + 播放/暂停/上一首/下一首/静音/**循环模式（列表循环 ⇄ 单曲循环）**）+ 全部曲目目录（目录号/曲名/作者/时长/播放中墨点）；**全局唯一 `<audio>`**（根布局 `MusicAudio` context，跨页播放不中断，ended 按循环模式切下一首或重播本曲，列表循环播完回第一首）；**打开网站自动播放**——挂载即尝试 play()，浏览器 autoplay 策略拒绝时以首个用户手势（点击/按键）兜底起播；键盘 Space 播放/暂停、←/→ 切曲、Esc 关闭；reduced-motion 跳过动画直接切换；曲目静态映射 `src/lib/music.ts`（10 首，ffprobe 实测时长）；mp3 在 `public/music/`
