...
- **首页结构（章节式长滚动叙事，2026-08-20 用户要求）**：线性纵向四章——**Ch.00 序**（档案图视差舞台 Hero，v0.21.3 替换 3D 波浪）→ **Ch.01 最近**（逐卡翻页，原样保留）→ **Ch.02 档案**（按年份分组的藏品图时间轴，档案元数据即排版元素）→ **Ch.03 落款**（花体手写签名 + 墨线 + 署名导航）。右侧竖向进度指示 + 章节菜单（`nav`/`button`/`aria-current`）。**转场节奏（2026-08-20 用户调优）**：wheel 平滑翻页 2s（easeInOut，可中断）；档案帖子**从右往左滑入、退场原路返回**（其他元素淡出）；落款手写描画 2.4s + 文字行错峰约 3s；档案/落款章配 Wellcome 蚀刻局部背景（multiply 水印底纹）。全部双向可逆、随 `prefers-reduced-motion` 降级为纯淡入。**演进纪律（防回滚，来自 v0.8.1/v0.11.1 两次整体回滚的根因）**：每一步增量独立可回滚、严格线性纵向、不推翻已认可成果；转场动效永远最后做，且必须随 `prefers-reduced-motion` 降级。实现细节见 `.claude/design/motion-and-interaction.md` 末节。
- **Hero 视差舞台（v0.21.3，用户否决 3D 水波纹后重建）**：全屏固定舞台——中央超大衬线主标题（两行 overflow-hidden 入场，行间 0.12s，无弹跳）+ 伊甸园蚀刻主图（Wellcome PDM，multiply 融入纸面）+ 底部 "SCROLL TO EXPLORE" 细线提示。**鼠标惯性视差**（normalized -1..1 + lerp 0.06，收敛即停）：背景 ±8/6 · 主图 ±18/12（rotateY ±1.5°/rotateX ±1°）· 主图内层反向 ±6/4 · 前景标题 ±28/20；鼠标离开平滑回中心。滚动离开时整台渐出上移衔接逐卡翻页。**3D 波浪（`wave-ocean.tsx`）已删除**——数字生成物与档案气质张力悬置项结案，three.js 依赖同步移除。
