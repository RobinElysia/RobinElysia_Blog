...
export function CommentForm({ postId }: { postId: number }) {
  const [result, action, isPending] = useActionState<FormState, FormData>(async (prev, fd) => {
    const r = await submitComment(prev, fd);
    return r.ok ? { ok: true, submitted: true } : { ok: false, error: r.error, submitted: false };
  }, initialState);
